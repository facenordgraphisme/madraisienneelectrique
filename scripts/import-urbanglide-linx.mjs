...
const HTML_FILE = path.join(__dirname, '..', 'src', 'articleblog', 'urbanglide-linx-160-test.html')
...
async function importArticle() {
  ...
  const title = titleMatch ? titleMatch[1].trim() : 'Test Urbanglide Linx 160'
  ...
  // Forcer le slug
  const slug = 'test-complet-urbanglide-linx-160'
  ...
  // Nettoyage: retirer header, hero image qui sont gérés par le layout
  content = content.replace(/<header>[\s\S]*?<\/header>/i, '')
  content = content.replace(/<img[^>]+class="hero"[^>]*\/>/i, '')
  // Retirer les blocs de scripts JSON-LD et le script de la FAQ
  content = content.replace(/<script\b[^>]*>([\s\S]*?)<\/script>/gi, '')
  content = content.trim()

  // Extraire les keyTakeaways
  const takeaways = []
  ...
  // Extraire la FAQ depuis le JSON-LD
  let faq = []
  ...
  try {
    ...
    const categoryId = 'category-tests'
